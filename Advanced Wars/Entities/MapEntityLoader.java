	...
	public static void mobaNeutralBuilding() {
		...
		for (int i = 0; i != xNeutralBuilding.length; i++) {
			for (Building x : Building.buildingsArray) {
				if (x.getxPosition() != xNeutralBuilding[i]
						&& x.getyPosition() != yNeutralBuilding[i]) {
					new NeutralBuilding(xNeutralBuilding[i],
							yNeutralBuilding[i]);
				}
			}
	...
	private void moba() {
		...
		for (int i = 0; i != xRedBuilding.length; i++) {
			...
			if (Terrain.terrainArray[xRedBuilding[i]][yRedBuilding[i]] != null) {
				Interface.tilePanel
						.remove(Terrain.terrainArray[xRedBuilding[i]][yRedBuilding[i]]);
			}
