public class PathFinders {
	...
	/**
	 * Display the selected Units movement action, or attack action range Can't
	 * be used to move or attack
	 */
	public static void Hover(Unit unit) {
		...
		if (!unit.isAttacked() && !unit.isMoved()
				|| !unit.getTeam().equals(TurnPanel.turnText)) {
			Interface.addUnitInfo(unit);
			if (unit.getType().equals("HELICOPTER")) {
				PathFinders.flyingHover(unit.getxPosition(),
						unit.getyPosition(), unit.info.movement,
						unit.info.range);
			} else {
				PathFinders.hover(unit.getxPosition(), unit.getyPosition(),
						unit.info.movement, unit.info.range);
			}
		} else if (!unit.isAttacked() && unit.isMoved()
				|| unit.getTeam() != TurnPanel.turnText) {
			PathFinders.hoverAttack(unit.getxPosition(), unit.getyPosition(),
					unit.info.range);
		}
	}
	...
	/**
	 * Display the selected Units movement action, or attack action range Can be
	...
	public static void selectedUnit() {
		...
		if (!Unit.selectedUnit.isAttacked() && !Unit.selectedUnit.isMoved()) {
			if (Unit.selectedUnit.getType().equals("HELICOPTER")) {
				PathFinders.fly(Unit.selectedUnit.getxPosition(),
						Unit.selectedUnit.getyPosition(),
						Unit.selectedUnit.info.movement);
			} else {
				PathFinders.move(Unit.selectedUnit.getxPosition(),
						Unit.selectedUnit.getyPosition(),
						Unit.selectedUnit.info.movement);
			}
		} else if (!Unit.selectedUnit.isAttacked()
				&& Unit.selectedUnit.isMoved()) {
			PathFinders.attack(Unit.selectedUnit.getxPosition(),
					Unit.selectedUnit.getyPosition(),
	...
	public static void selectedUnitAttack() {
		PathFinders.attack(Unit.selectedUnit.getxPosition(),
				Unit.selectedUnit.getyPosition(), Unit.selectedUnit.info.range);
