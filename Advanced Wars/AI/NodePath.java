...
public class NodePath {
	...
	Node[][] map = new Node[TablePanel.HEIGHT][TablePanel.WIDTH];
	...
	public NodePath(Unit start, Point cord) {
		...
		startNode = new Node(start.getxPosition(), start.getyPosition(),
				endNode);
	...
	private final void addToPathLoopParents(Node node) {
		Terrain.terrainArray[node.getxPosition()][node.getyPosition()]
				.highlight();
		path.add(node);
	...
	private final void expand(Node expand) {
		...
		try {
			map[expand.getxPosition() + 1][expand.getyPosition()]
					.setParent(expand);
		} catch (Exception e) {
		...
		try {
			map[expand.getxPosition()][expand.getyPosition() + 1]
					.setParent(expand);
		} catch (Exception e) {
	...
	private void initMap() {
		...
		for (int x = 0; x < TablePanel.WIDTH; x++) {
			...
			for (int y = 0; y < TablePanel.HEIGHT; y++) {
				...
				} else if (x == startNode.getxPosition()
						&& y == startNode.getyPosition()) {
					map[x][y] = startNode;
	...
	private final void restart() {
		...
		startNode = new Node(startNode.getxPosition(),
				startNode.getyPosition(), endNode);

		initMap();

		for (Unit unit : Unit.UnitsArray) {
			if (unit.isMoved())
				map[unit.getxPosition()][unit.getyPosition()]
